...
async function addGatePass(req,res){
    ...
    let gate_pass_currency = req.body.gate_pass_currency;
    ...
    if(isGp == true){
        ...
        var query1 = "insert into gate_pass(gp_number_manual,gp_party_id,gp_type,gp_date,gp_contact, gp_total, gp_payment_type, gp_currency) values('"+gp_number_manual+"','"+gate_pass_party_id+"','"+gp_type+"','"+gp_date+"','"+gp_contact+"','"+gate_pass_grand_total+"','"+gp_payment_type+"', '"+gate_pass_currency+"')";
        app.conn.query(query1, async function(err,result){
            if(err){
                ...
                for(let i=0; i<gp_row.length; i++){
                    ...
                    ledgerData.l_currency = gate_pass_currency
    ...
    return new Promise(function(resolve,reject){
        ...
        app.conn.query(query1, function(err,result1){
            ...
            for(let i=0; i<=data.length; i++){   
                if(i<data.length){
                    query2 = "insert into ledger(party_id,gp_number,gp_number_manual,l_commodity,l_description,l_seller_weight,l_buyer_weight,l_rate,l_debit,l_credit,l_balance,l_date,l_type, l_currency, l_unit) values('"+data[i].party_id+"','"+data[i].gp_number+"','"+data[i].gp_number_manual+"','"+data[i].l_commodity+"','"+data[i].l_description+"','"+data[i].l_seller_weight+"','"+data[i].l_buyer_weight+"','"+data[i].l_rate+"','"+data[i].l_debit+"','"+data[i].l_credit+"','"+data[0].l_balance+"','"+data[i].l_date+"','"+data[i].l_type+"', '"+data[i].l_currency+"', '"+data[i].l_unit+"')"
                    app.conn.query(query2, function(err,result){
                        if(err){
                            console.log("Error at ledger entry: "+err.message)
                        }
                    })
