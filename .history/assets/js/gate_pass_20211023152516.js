...
idList = [idCounter];
let data = [];
let gp_number_manual, gate_pass_date, gate_pass_party_id, gate_pass_party_name, gate_pass_type, gate_pass_currency, gate_pass_grand_total, gate_pass_contact = "";
...
inputAmountType.addEventListener('change', (event)=>{
    ...
    if(type == "KGS"){
        ...
        quantity = parseFlaot(inputQuantity.value/37.324)
        inputTotalAmount.value = quantity * inputUnitAmount.value
        getGrandTotal()
    } else if (type == "TONS"){
        quantity = parseFlaot(inputQuantity.value/1000)
        inputTotalAmount.value = quantity * inputUnitAmount.value
        getGrandTotal()
    }
})
    ...
    if(getGatePass()){
        ...
        fetch("/gate_pass/add-gate-pass", {
            ...
            body: JSON.stringify({ 
                                    ...
                                    "gate_pass_currency" : gate_pass_currency,
...
function getGatePass(){
    ...
    gate_pass_currency = document.getElementById("gate_pass_currency").value;
