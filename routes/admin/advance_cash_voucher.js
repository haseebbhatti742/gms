...
router.post('/add', async (req, res) => {
    ...
    cv_currency = req.body.cv_currency
    ...
    if(isCV == true){
        ...
        addCashVoucher(cash_voucher_number_manual,cv_commodity,party_id,cv_date,cv_type,cv_payment_type,cv_contact,cv_name,cv_signature,cv_amount, cv_details, cv_currency, res)
    }
});
...
let cv_number
function addCashVoucher(cash_voucher_number_manual,cv_commodity,party_id,cv_date,cv_type,cv_payment_type,cv_contact,cv_name,cv_signature,cv_amount, cv_details, cv_currency, res){
    let ledgerData = []
    ...
    ledgerData.l_currency = cv_currency
    ...
    query1 = "insert into cash_voucher (cv_number_manual,cv_commodity,party_id, cv_date, cv_type, cv_payment_type, cv_name, cv_signature, cv_amount, cv_details,cv_contact,cv_advance, cv_currency) values ('"+cash_voucher_number_manual+"','"+cv_commodity+"','"+party_id+"', '"+cv_date+"', '"+cv_type+"', '"+cv_payment_type+"', '"+cv_name+"', '"+cv_signature+"', '"+cv_amount+"', '"+cv_details+"', '"+cv_contact+"','true', '"+cv_currency+"')"
...
function addIntoLedgerWithCV(data){
    return new Promise(function(resolve,reject){
        ...
        app.conn.query(query1, function(err,result1){
            ...
            query1 = "insert into ledger(cv_number_manual,party_id,cv_number,l_commodity,l_description,l_seller_weight,l_buyer_weight,l_rate,l_debit,l_credit,l_balance,l_date,l_type, l_currency) values('"+data.cv_number_manual+"','"+data.party_id+"','"+data.cv_number+"','"+data.l_commodity+"','"+data.l_description+"','"+data.l_seller_weight+"','"+data.l_buyer_weight+"','"+data.l_rate+"', '"+data.l_debit+"','"+data.l_credit+"','"+data.l_balance+"','"+data.l_date+"','"+data.l_type+"', '"+data.l_currency+"')"
